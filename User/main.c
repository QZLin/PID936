...
volatile u8 parseLen = 0;
volatile uint16_t triggerCounter = 1;
//
...
static inline void SetGPIO(GPIO_TypeDef *GPIOx, GPIO_InitTypeDef def, const uint32_t pin) {
    def.GPIO_Pin = pin;
    GPIO_Init(GPIOx, &def);
}
 ...
 * @return  none
 */
static inline void GPIO_CFG(void) {
...
static inline void ADC_CFG(void) {
...
static inline void EXTI_CFG(void) {
    GPIO_InitTypeDef GPIO_InitStructure = {0};
...
static inline void TIM2_Init(void) {
    ...
    // 时钟已经在 RCC_Cfg 中开启 TIM2

    // 配置时基单元
    TIM_TimeBaseInitStruct.TIM_Period = triggerCounter; // 自动重装载值 (10kHz/10 = 1kHz -> 1ms)
...
void taskShowTemp(void *pvParameters) {
    ...
    for (;;) {
        ...
        const TickType_t x_ticks_to_delay = pdMS_TO_TICKS(parseLen * 50);
        if (xSemaphoreTake(x595Mutex, portMAX_DELAY)) {
            hc595Data |= TEMP_LED;
            xSemaphoreGive(x595Mutex);
        }
        vTaskDelay(x_ticks_to_delay);
        if (xSemaphoreTake(x595Mutex, portMAX_DELAY)) {
            hc595Data &= ~TEMP_LED;
            xSemaphoreGive(x595Mutex);
        }
...
__attribute__((interrupt())) void USART2_IRQHandler(void) {
    if (USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) {
        const uint8_t data = USART_ReceiveData(USART2);
    ...
    if (MASK(0b10000000)) {
        ...
        if (MASK(0b00000001)) {
            ...
            if (HandlerHello == NULL) {
                printf("OwO\r\n");
                xTaskCreate(taskHello, "OwO",TASK_STK_SIZE4,NULL,PRIO_DISPLAY, &HandlerHello);
            }
...
#define TSK(func,handler,depth,priority) xTaskCreate(func,#func,depth,NULL,priority,handler)
    ...
    TSK(taskControl, &HandlerControl, TASK_STK_SIZE1, PRIO_INPUT);
    TSK(taskHC165In, &Handler165IN, TASK_STK_SIZE1, PRIO_INPUT);
