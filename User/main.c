...
void taskShowTemp(void* pvParameters)
{
    ...
    for (;;)
    {
        ...
        const TickType_t x_ticks_to_delay = pdMS_TO_TICKS(100);
...
#define MASK(bin) ((v & bin) == 0)
    ...
    else if (MASK(BTN_DBG)) // DBG
    {
        ...
        if (MASK(BTN_RST))
        {
            ...
            {
                printf("OwO\r\n");
                xTaskCreate(taskHello, "OwO",TASK_STK_SIZE3,NULL,PRIO_DISPLAY, &HandlerHello);
            }
        }
    ...
    {
        if (state.dstTemp < MAX_TEMP)
            state.dstTemp += STEP_TEMP;
        printf("T:Temp=%u\r\n", state.dstTemp);
    }
    else if (MASK(BTN_SUB))
    {
